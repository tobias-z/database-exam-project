...
enum Query {
    ...
    FzyFind(command::FzyFindCommand),
    RegexFind(command::RegexFindCommand),
...
impl Query {
    fn execute(&self, pipe: Vec<&str>, push: &mut impl FnMut(Document)) -> anyhow::Result<()> {
        match self {
            ...
            Query::FzyFind(cmd) => cmd.execute(pipe, push),
            Query::RegexFind(cmd) => cmd.execute(pipe, push),
...
impl FromStr for Query {
    ...
    fn from_str(query_command: &str) -> Result<Self, Self::Err> {
        match query_command {
            ...
            "fzy_find" | "FZY_FIND" => Ok(Query::FzyFind(command::FzyFindCommand)),
            "find" | "FIND" => Ok(Query::RegexFind(command::RegexFindCommand)),
