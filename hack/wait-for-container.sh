...
function wait_for_container {
    ...
    # exit if this container has not health check
    if ! docker inspect "${container_id}" --format '{{ .State.Health.Status }}'; then
        echo "Skipping container ${container_name} because it has not healthcheck"
        exit 0
    fi
